...
classifier = xgboost(data = data, label = label, nrounds = 10, num_class = 7, objective = "multi:softmax", verbose = 2)

# Predicting the Test set results
newdata = as.matrix(test_set[,-18])
# mode(newdata) <- 'double'
y_pred = predict(classifier, newdata = newdata)
y_pred

cm2 = table(as.matrix(test_set[, 18]), y_pred)
cm2
...
cv = lapply(folds, function(x) {
  ...
  classifier = xgboost(data = as.matrix(training_set[,-18]), label = training_set$NObeyesdad, nrounds = 20, num_class = 7, objective = "multi:softmax", verbose = 2)
  y_pred = predict(classifier, newdata = as.matrix(test_fold[,-18]))
  # y_pred = (y_pred >= 0.5) Only needed for binomial classification
  cm = table(as.matrix(test_fold[, 18]), y_pred)
